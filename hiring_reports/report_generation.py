...
COMPANIES_ROOT = Path(os.getenv("COMPANIES_ROOT", "job_postings/pre_processed")).expanduser()
COMPANIES_ROOT.mkdir(parents=True, exist_ok=True)

PROCESSED_ROOT = Path(os.getenv("PROCESSED_ROOT", "job_postings/processed")).expanduser()
PROCESSED_ROOT.mkdir(parents=True, exist_ok=True)

REPORTS_ROOT = Path(os.getenv("REPORTS_ROOT", "hiring_reports/reports")).expanduser()
...
print(f"Input workbooks root: {COMPANIES_ROOT.resolve()}")
print(f"Processed workbooks root: {PROCESSED_ROOT.resolve()}")
print(f"Hiring reports root: {REPORTS_ROOT.resolve()}")
print(f"Model: {MODEL}")
...
def find_source_workbook(company_path: Path) -> Path:
    if company_path.is_file() and company_path.suffix.lower() == ".xlsx":
        if company_path.name.startswith("~$"):
            raise FileNotFoundError(f"Temporary Excel file is not a valid input: {company_path}")
        return company_path

    files = [
        path for path in sorted(company_path.glob("*.xlsx"))
    ...
    if not files:
        ...
        raise FileNotFoundError(f"No .xlsx file found in {company_path}")
    return files[0]
...
def get_output_workbook_path(source_path: Path) -> Path:
    if source_path.stem.endswith("_enriched"):
        return source_path
    return PROCESSED_ROOT / f"{source_path.stem}_enriched.xlsx"


def get_report_output_dir(source_path: Path) -> Path:
    company_slug = re.sub(r"[^A-Za-z0-9_.-]+", "_", source_path.stem).strip("_") or "company"
    out_dir = REPORTS_ROOT / company_slug
...
def run_company_pipeline(company_path: Path) -> dict:
    source_path, output_path, jobs = load_jobs_for_processing(company_path)
    report_output_dir = get_report_output_dir(source_path)

    print()
    print(f"=== {source_path.stem} ===")
    print(f"Input workbook: {source_path}")
    print(f"Output workbook: {output_path}")
    print(f"Report folder: {report_output_dir}")
    ...
    if RUN_ANALYSIS:
        pdf_paths = run_analysis_reports(enriched_jobs, report_output_dir)

    return {
        "company": source_path.stem,
        "input_workbook": str(source_path),
        "output_workbook": str(output_path),
        "report_folder": str(report_output_dir),
...
def resolve_company_arg(arg: str) -> Path:
    ...
    if candidate.exists():
        return candidate
    if not arg.lower().endswith(".xlsx"):
        candidate_xlsx = COMPANIES_ROOT / f"{arg}.xlsx"
        if candidate_xlsx.exists():
            return candidate_xlsx
    raise FileNotFoundError(f"Could not find company workbook or folder from argument: {arg}")


def discover_company_inputs() -> list[Path]:
    workbooks = [
        path for path in sorted(COMPANIES_ROOT.glob("*.xlsx"))
        if not path.name.startswith("~$") and not path.stem.endswith("_enriched")
    ]
    if workbooks:
        return workbooks
...
def main():
    if len(sys.argv) > 1:
        company_inputs = [resolve_company_arg(arg) for arg in sys.argv[1:]]
    else:
        company_inputs = discover_company_inputs()

    if not company_inputs:
        print(f"No company Excel workbooks or folders found inside {COMPANIES_ROOT}/")
        return

    summary_rows = []
    total_companies = len(company_inputs)

    print(f"Found {total_companies} company inputs.")

    for company_number, company_input in enumerate(company_inputs, start=1):
        print()
        print("=" * 100)
        print(f"Company {company_number}/{total_companies}: {company_input.stem if company_input.is_file() else company_input.name}")
        print("=" * 100)

        try:
            summary_rows.append(run_company_pipeline(company_input))
        except Exception as exc:
            summary_rows.append(
                {
                    "company": company_input.stem if company_input.is_file() else company_input.name,
                    "input_workbook": None,
            ...
            print(f"Error in {company_input}: {exc}")
